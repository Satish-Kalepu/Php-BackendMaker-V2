	...
	<div style="position: fixed;left:150px; top:80px; height: 40px; width:calc( 100% - 150px ); background-color: white; overflow: hidden; " >
			<div class="tabs_nav_bar">
				<div class="tabs_nav_container" id="tabs_container">
					<div v-bind:class="{'tab_btn':true, 'tab_btn_active':(tab=='script')}" >
						<div v-on:click.prevent.stop="open_tab('script')">PHP Script</div>
					</div>
					<div v-bind:class="{'tab_btn':true, 'tab_btn_active':(tab=='help')}" >
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">SDK Settings</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body" >
						<div class="mb-2">Name</div>
						<div><input type="text" v-model="main_sdk__['name']" class="form-control form-control-sm" v-on:change="nchange" ></div>
						<div class="text-secondary small mb-2">no spaces. no special chars. except dash(-/_). Title case recommended</div>
						<div class="mb-2">Description</div>
						<div class="mb-2"><textarea v-model="main_sdk__['des']" class="form-control form-control-sm" ></textarea></div>

						<div class="mb-2">Keywords</div>
						<div v-for="kv,ki in main_sdk__['keywords']" style="display:flex; column-gap:10px;" >
							<div style="width:200px;"><input type="text" v-model="main_sdk__['keywords'][ki]" class="form-control form-control-sm" placeholder="keyword" v-on:blur="clean_keyword(ki)"></div>
						...
						<div class="mb-2">&nbsp;</div>
						<div class="mb-2">
							<input type="button" class="btn btn-outline-dark btn-sm" value="UPDATE" v-on:click="save_settings()" >
						</div>
						<div class="text-danger" v-if="set_err">{{ set_err }}</div>
						<div class="text-success" v-if="set_msg">{{ set_msg }}</div>
						<div class="mb-2">&nbsp;</div>
				</div>
			</div>
		</div>
