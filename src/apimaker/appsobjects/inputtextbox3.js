	...
	template:`<div v-bind:class="'codeline_thing codeline_thing_'+v['t']" >
		...
		<div v-if="v['t']=='GT'" >
			<template v-if="typeof(v['v'])=='object'" >
				<div title="Thing" data-type="dropdown" v-bind:data-var="datavar+':v'" data-list="graph-thing" v-bind:data-thing="'GT-ALL'" data-thing-label="Things" >{{ v['v'] }}</div>
				<a v-if="v['i']['v']" href="#" class="codeline_gt_link" >{{ v['i']['v'] }}</a>
			</template>
			<div v-else>GT value need object</div>
		</div>
		<div v-else-if="v['t']=='T'" title="Text" class="editable"  v-bind:data-var="datavar+':v'" ><div contenteditable spellcheck="false" data-type="editable"  v-bind:data-var="datavar+':v'" v-bind:id="datavar+':v'" v-bind:data-allow="v['t']"  >{{ v['v'] }}</div></div>
