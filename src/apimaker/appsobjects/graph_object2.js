const graph_object2 =  {
	...
	watch: {
		v: {
			handler: function(){
				//this.$emit("updated", this.$root.thing);
				console.log("updated");
				this.saveit = true;
			}, deep:true
		}
	...
	mounted: function(){
		if( typeof(this.$root.thing) != "object" || "length" in this.$root.thing ){
			this.$root.thing = {};
		}
		if( "i_of" in this.$root.thing == false || typeof(this.$root.thing['i_of']) != "object" || "length" in this.$root.thing['i_of'] ){
			this.$root.thing['i_of'] = {
				"t":"GT", 
				"v":{"t":"T","v":""},
				"i":{"t":"T","v":""}
			};
		}
		if( "p_of" in this.$root.thing == false || typeof(this.$root.thing['p_of']) != "object" || "length" in this.$root.thing['p_of'] == false ){
			this.$root.thing['p_of'] = [{
				"t":"GT", 
				"v":{"t":"T","v":""},
				"i":{"t":"T","v":""}
			}];
		}
		if( "al" in this.$root.thing == false || typeof(this.$root.thing['al']) != "object" || "length" in this.$root.thing['al'] == false ){
			this.$root.thing['al'] = [];
		}
		if( "props" in this.$root.thing == false ){
			this.$root.thing['props'] = {};
		}else if( typeof(this.$root.thing['props']) != "object" || "length" in this.$root.thing['props'] ){
			this.$root.thing['props'] = {};
		}
		if( "z_t" in this.$root.thing['i_of'] == false ){
			this.$root.thing['i_of']['z_t'] = {
				//"p1":{"l":{"t":"T","v":"Description"}, "t":{"t":"KV", "v":"Text", "k":"T"}, "e":false, "m":false},
			};
			this.$root.thing['i_of']['z_o'] = [];
			this.$root.thing['i_of']['z_n'] = 1;
		}else if( typeof(this.$root.thing['i_of']['z_t']) != "object" || "length" in this.$root.thing['i_of']['z_t'] ){
			this.$root.thing['i_of']['z_t'] = {
				//"p1":{"l":{"t":"T","v":"Description"}, "t":{"t":"KV", "v":"Text", "k":"T"}, "e":false, "m":false},
			};
			this.$root.thing['i_of']['z_o'] = [];
	...
	methods: {
		...
		create_field: function(){
			...
			f = f.replace(/xDOTx/g, '.');
			//if( )
			for(var k in this.$root.thing['z_t']['v']){
				if( this.$root.thing['z_t']['v'][k]['v'].toLowerCase() == f.toLowerCase() ){
					alert("Field `" + f + "` already exists");
					return false;
				}
			}
			var nf = 'p1';var n = 1;
			while( nf in this.$root.thing['z_t']['v'] ){
				n++; nf = 'p'+n;
			}
			this.$root.thing['props']['v'][ nf ] = {'t':"L", "v": [v]};
			this.$root.thing['z_t']['v'][ nf ] = {'t':v['t']+'', 'v':f+''};
			if( 'z_t' in this.$root.thing['i_of']['v'][0] == false ){
				this.$root.thing['i_of']['v'][0]['z_t'] = {'t':"O", "v":{}};
			}
			this.$root.thing['i_of']['v'][0][ 'z_t' ]['v'][ nf ] = {'t':v['t']+'', 'v':f+''};
			this.$root.temp = {"new_field": {'t':"T",'v':""}, "new_value": {'t':"T",'v':""}};
			this.tab="home";
		},
		add_sub: function(vv,vk,vi){
			if( vk in this.$root.thing[vv] == false ){
				this.$root.thing[vv][ vk ] = [];
			}
			if( this.$root.thing[vv][ vk ].length>0 ){
				var v = JSON.parse(JSON.stringify( this.$root.thing[vv][ vk ][0] ));
			}else{
				var v = {'t':"T", "v":""};
			}
			this.$root.thing[vv][ vk ].splice( vi, 0, v );
		},
		del_sub: function(vv,vk,vi){
			if( confirm("Are you sure?") ){
				this.$root.thing[vv][ vk ].splice(vi,1);
			}
		},
		add_i_of: function(){
			if( this.$root.thing['i_of']['v'].length < 4 ){
				this.$root.thing['i_of']['v'].push({
					"t":"GT", 
					"v":{"t":"T","v":""},
					"i":{"t":"T","v":""}
				});
		...
		del_i_of: function(vi){
			if( this.$root.thing['i_of']['v'].lenth <= 1 ){
				alert("need at least one item");return false;
			}
			this.$root.thing['i_of']['v'].splice(vi,1);
		},
		add_field: function(){
			...
			var np = "p1";
			for(var i=Number(this.$root.thing['i_of']['z_n']);i<999;i++){
				var np = "p"+i;
				if( np in this.$root.edit_z_t == false ){
		...
		save_new_field: function(){
			...
			axios.post("?", {
				...
				"object_id": this.$root.thing['i_of']['i'],
					...
					if( typeof( response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								...
								this.z_t_msg = "Saved";
								this.$root.edit_z_t[ this.new_field ] = {
									"l": JSON.parse(JSON.stringify(this.temp['new_field'])),
									"t": JSON.parse(JSON.stringify(this.temp['new_type'])),
									"e": false,
									"m": false
								...
								this.$root.thing['i_of']['z_t'] = JSON.parse(JSON.stringify(this.$root.edit_z_t));
								this.$root.thing['i_of']['z_o'] = JSON.parse(JSON.stringify(this.edit_z_o));
								this.$root.thing['i_of']['z_n'] = this.new_field_id+0;
								this.$root.temp = {
									"new_field": {"t":"T", "v":""}, 
		...
		open_template_edit: function(){
			this.edit_template = this.$root.thing['i_of']['i'];
			this.this.$root.edit_z_t = JSON.parse(JSON.stringify(this.$root.thing['i_of']['z_t']));
			this.edit_z_o = JSON.parse(JSON.stringify(this.$root.thing['i_of']['z_o']));
			this.tab="template";
		},
		...
		open_props_edit: function(){
			this.edit_template = this.$root.thing['i_of']['i'];
			console.log('333');
			console.log( JSON.parse(JSON.stringify(this.$root.edit_z_t)) );
			console.log('444');
		...
		close_edit_field: function(){
			this.edit_field = '';
			this.$root.thing['i_of']['z_t'] = JSON.parse(JSON.stringify(this.$root.edit_z_t));
			this.$root.thing['i_of']['z_o'] = JSON.parse(JSON.stringify(this.$root.edit_z_t));
		},
		save_field: function(){
			var v = this.$root.edit_z_t[ this.edit_field ]['l']['v']+'';
			for(var fd in this.$root.edit_z_t ){
				if( fd != this.edit_field && this.$root.edit_z_t[ fd ]['l']['v'].toLowerCase() == v.toLowerCase() ){
		...
		save_z_t: function(){
			...
			axios.post("?", {
				...
				"prop": this.$root.edit_z_t[ this.edit_field ],
			}).then(response=>{
				this.msg = "";
				if( response.status == 200 ){
					if( typeof( response.data) == "object" ){
					...
					if( typeof( response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								...
								var i = this.$root.thing['i_of']['z_o' ].indexOf( this.delete_field_id );
								this.$root.thing['i_of']['z_o' ].splice( i, 1 );
								delete(this.$root.thing['i_of']['z_t' ][ this.delete_field_id ]);
								this.this.$root.edit_z_t = JSON.parse( JSON.stringify(this.$root.thing['i_of']['z_t']) );
								this.edit_z_o = JSON.parse( JSON.stringify(this.$root.thing['i_of']['z_o']) );
		...
		save_props: function(){
			for( var pv in this.$root.thing['props'] ){
				if( this.$root.thing['i_of']['z_t'][ pv ]['t']['k'] == "O" ){
					var propd = this.$root.thing['i_of']['z_t'][ pv ]['z'];
					for( var i=0;i<this.$root.thing['props'][ pv ].length;i++ ){
						var f = false;
						for( var fd in propd['z_t'] ){
							if( fd in this.$root.thing['props'][ pv ][i] ){
								if( this.$root.thing['props'][ pv ][i][ fd ]['v'].trim() != "" ){
									f= true;
								}
							}
						}
						if( f == false ){
							this.$root.thing['props'][ pv ].splice(i,1);
							i--;
						}
					}
				}else{
					for( var i=0;i<this.$root.thing['props'][ pv ].length;i++ ){
						if( this.$root.thing['props'][ pv ][i]['v'].trim() == "" ){
							this.$root.thing['props'][ pv ].splice(i,1);
							i--;
						}
					}
		...
		open_edit_label: function(){
			this.$root.thing['edit_label_v'] = JSON.parse( JSON.stringify(this.$root.thing['l']) );
			this.edit_label = true;
		},
		open_edit_al: function(){
			this.$root.thing['edit_al_v'] = JSON.parse( JSON.stringify(this.$root.thing['al']) );
			this.edit_al = true;
		},
		open_edit_i_of: function(){
			this.$root.thing['edit_i_of_v'] = JSON.parse( JSON.stringify(this.$root.thing['i_of']) );
			this.edit_i_of = true;
		},
		save_al: function(){
			for(var i=0;i<this.$root.thing['edit_al_v'].length;i++){
				if( this.$root.thing['edit_al_v'][i]['v'].trim()=="" ){
					this.$root.thing['edit_al_v'].splice(i,1);i--;
				}else if( this.$root.thing['edit_al_v'][i]['v'].toLowerCase() == this.$root.thing['l']['v'].toLowerCase() ){
			...
			axios.post("?", {
				...
				"object_id": this.$root.thing['_id'],
				"alias": this.$root.thing['edit_al_v'],
			}).then(response=>{
				this.al_msg = "";
				if( response.status == 200 ){
					if( typeof( response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								this.al_msg = "Updated";
								this.$root.thing['al'] = JSON.parse( JSON.stringify(this.$root.thing['edit_al_v']));
								this.edit_al = false;
		...
		save_label: function(){
			...
			axios.post("?", {
				...
				"object_id": this.$root.thing['_id'],
				"label": this.$root.thing['edit_label_v'],
			}).then(response=>{
				this.label_msg = "";
				if( response.status == 200 ){
					if( typeof( response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								...
								this.$root.thing['l'] = JSON.parse( JSON.stringify(this.$root.thing['edit_label_v']));
		...
		save_i_of: function(){
			...
			axios.post("?", {
				...
				"object_id": this.$root.thing['_id'],
				"i_of": this.$root.thing['edit_i_of_v'],
			}).then(response=>{
				this.i_of_msg = "";
				if( response.status == 200 ){
					if( typeof( response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								...
								this.$root.thing['i_of'] = JSON.parse( JSON.stringify(this.$root.thing['edit_i_of_v']));
		...
		add_al: function(){
			this.$root.thing['edit_al_v'].push({"t":"T","v":""});
		},
		del_al: function(vi){
			if( confirm("Are you sure?") ){
				this.$root.thing['edit_al_v'].splice(vi,1);
			}
		},
		callback__: function( c ){
			var x = c.split(/\:/g);
			if( x[0] == "z_t_type" ){
				this.echo__( this.$root.edit_z_t[ x[1] ] );
				if( this.$root.edit_z_t[ x[1] ]['t']['k'] == "O" ){
					this.$root.edit_z_t[ x[1] ]['z'] = {
						"z_t": {
							"p1": {
								"l": "Field 1",
								"t": "T"
					...
					delete(this.$root.edit_z_t[ x[1] ]['z']);
				}
			}else{
		...
		add_object_field: function(){
			this.$root.edit_z_t[ this.edit_field ]['z']['z_n']++;
			var n = this.$root.edit_z_t[ this.edit_field ]['z']['z_n']+0;
			var new_p = "p"+n;
			this.$root.edit_z_t[ this.edit_field ]['z']['z_t'][ new_p ] = {
				"l": "Field "+n,
				"t": "T"
			};
			this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].push( new_p );
		},
		object_field_moveup: function(vf){
			var i = this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].indexOf(vf);
			if( i > 0 ){
				var x = this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].splice(i,1);
				this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].splice(i-1,0,x[0]);
			}
		},
		object_field_movedown: function(vf){
			var i = this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].indexOf(vf);
			if( i < this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].length-1 ){
				var x = this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].splice(i,1);
				this.$root.edit_z_t[ this.edit_field ]['z']['z_o'].splice(i+1,0,x[0]);
			}
		},
		add_sub_object: function( propf ){
			var o = {};
			var propd = this.$root.thing['i_of']['z_t'][ propf ];
			for(var tdi in propd['z']['z_t']){
				o[ tdi ] = {"t":"T", "v":""};
			}
			this.$root.thing['props'][ propf ].push( o );
		},
		del_sub_object: function( propf, vi ){
			this.$root.thing['props'][ propf ].splice( vi,1 );
		}
	},
	template: `<div class="code_line">
		<div v-if="$root.thing_id==-1" >
			<div>need thing id</div>
		</div>
		<template v-else >
			<div v-if="'l' in $root.thing==false" >Loading...</div>
			<div v-else >
			...
			<tr>
				...
				<td>
					<div v-if="edit_label==false" style="display:flex; column-gap:20px;" >
						<div v-if="$root.thing['l']['t']=='GT'" >
							<template v-if="'v' in $root.thing['l']&&'i' in $root.thing['l']" >
								<a href="#" v-on:click.prevent.stop="getlink($root.thing['l']['i'])" style="font-size:1.2rem;" >{{ $root.thing['l']['v'] }}</a>
							</template>
							<div v-else>error object</div>
						</div>
						<div v-else style="font-size:1.2rem;" >{{ $root.thing['l']['v'] }}</div>
					...
					<div v-if="edit_label==true" >
						<div style="display:flex; column-gap:20px; border:1px solid #ccc; padding:10px;" >
							<inputtextbox2 types="T,GT" v-bind:v="$root.thing['edit_label_v']" v-bind:datavar="datavar+':edit_label_v'" ></inputtextbox2>
							<div>
			...
			<tr>
				...
				<td>
					<div v-if="edit_al==false" style="display:flex; column-gap:20px;" >
						<div>
							<div v-for="alv in $root.thing['al']">{{ alv['v'] }}</div>
						</div>
					...
					<div v-if="edit_al==true" >
						<div style="display:flex; column-gap:20px; border:1px solid #ccc; padding:10px;" >
							<div>
								<div v-for="alv,ali in $root.thing['edit_al_v']" style="display:flex; column-gap:5px; border-bottom:1px dashed #ccc; align-items:center;" >
			...
			<tr>
				...
				<td>
					<div v-if="edit_i_of==false" style="display:flex; column-gap:20px;" >
						<div v-if="$root.thing['i_of']['t']=='GT'" >
							<template v-if="'v' in $root.thing['i_of']&&'i' in $root.thing['i_of']" >
								<a href="#" v-on:click.prevent.stop="getlink($root.thing['i_of']['i'])" >{{ $root.thing['i_of']['v'] }}</a>
							</template>
							<div v-else>error object</div>
						</div>
					...
					<div v-if="edit_i_of==true" >
						<div style="display:flex; column-gap:20px; border:1px solid #ccc; padding:10px;" >
							<inputtextbox2 types="GT" v-bind:v="$root.thing['edit_i_of_v']" v-bind:datavar="datavar+':edit_i_of_v'" ></inputtextbox2>
				...
				<td>
					<template v-if="'p_of' in v" >
						<div v-if="'p_of' in v" class="codeline_thing" style="display:flex;column-gap:10px;" >
							<div>
								<div v-if="typeof($root.thing['p_of']['v'])!=undefined" title="Thing" data-type="dropdown" v-bind:data-var="datavar+':p_of:v'"  data-list="graph-thing" v-bind:data-thing="'GT-ALL'" data-thing-label="Things" >{{ $root.thing['p_of']['v']['l']['v'] }}</div>
								<div v-else>$root.thing['p_of']['v'] Undefined</div>
							</div>
						</div>
					</template>
					<div>Undefined</div>
		...
		<ul class="nav nav-tabs">
			...
			<li class="nav-item" v-if="'cnt' in v">
				<a v-if="$root.thing['cnt']>0" v-bind:class="{'nav-link py-0':true, 'active':tab=='records'}"  href="#" v-on:click.prevent.stop="open_records()">Records <span class="badge bg-secondary" >{{ $root.thing['cnt'] }}</span> </a>
			</li>
		</ul>
		<template v-if="vedit==false&&tab=='home'" >
			...
			<table v-if="'z_o' in $root.thing['i_of']" class="table table-bordered table-sm w-auto" >
				<tbody>
				<tr v-for="propf in $root.thing['i_of']['z_o']" valign="top">
					<td nowrap>
						<span v-if="propf in $root.thing['i_of']['z_t']" >{{ $root.thing['i_of']['z_t'][ propf ]['l']['v'] }}</span>
						<span v-else >{{ propf }}</span>
					</td>
					<td>
						<div v-if="propf in $root.thing['props']==false" >-</div>
						<div v-else-if="typeof($root.thing['props'][ propf ])!='object'&&'length' in $root.thing['props'][ propf ]==false" >-</div>
						<div v-else-if="$root.thing['props'][ propf ].length>0" >
							<template v-if="$root.thing['i_of']['z_t'][ propf ]['t']['k']=='O'" >
								<table class="table table-bordered table-striped table-sm w-auto customborder2" >
									<tbody>
										<tr>
											<td v-for="tdv in $root.thing['i_of']['z_t'][ propf ]['z']['z_o']" >{{ $root.thing['i_of']['z_t'][ propf ]['z']['z_t'][ tdv ]['l'] }}</td>
										</tr>
										<tr v-for="pvv,pii in $root.thing['props'][ propf ]" >
											<td v-for="tdv in $root.thing['i_of']['z_t'][ propf ]['z']['z_o']" >
							...
							<template v-else >
								<div v-for="pvv,pii in $root.thing['props'][ propf ]" >
		...
		<template v-if="vedit==true&&tab=='home'&&'z_o' in $root.thing['i_of']" >
			...
			<table class="table table-bordered table-sm w-auto customborder2" >
				...
				<tr v-for="propf in $root.thing['i_of']['z_o']" valign="top">
					...
					<td>
						...
						<template v-else >
							<div v-if="propf in $root.thing['props']==false" >
							...
							<div v-else >
								<div v-for="pvv,pii in $root.thing['props'][ propf ]" style="display:flex; column-gap:5px; border-bottom:1px dashed #ccc; align-items:center;" >
			...
			<div v-if="$root.thing['i_of']['z_o'].length>5">
		...
		<template v-if="tab=='template'" >
			<div style="padding:10px 0px; background-color:#f8f8f8;">Properties of {{ $root.thing['i_of']['v'] }}</div>
			<table class="table table-bordered table-sm w-auto" >
				<tbody>
				...
				<tr>
					<td><div>{{ propf }}</div></td>
					<td>
						<div v-if="propf in $root.edit_z_t" >
							<div v-if="edit_field==propf" >
								<inputtextbox2 types="T,GT" v-bind:v="$root.edit_z_t[ propf ]['l']" v-bind:datavar="'edit_z_t:'+propf+':l'" ></inputtextbox2>
							</div>
							<div v-else-if="$root.edit_z_t[ propf ]['l']['t']=='T'">{{ $root.edit_z_t[ propf ]['l']['v'] }}</div>
							<div v-else-if="$root.edit_z_t[ propf ]['l']['t']=='GT'"><a href="#" v-on:click.prevent.stop="getlink($root.edit_z_t[ propf ]['l']['i'])" >{{ $root.edit_z_t[ propf ]['l']['v'] }}</a></div>
						</div>
					</td>
					<td>
						<div v-if="propf in $root.edit_z_t" >
							<div v-if="edit_field==propf" title="DataType" data-type="dropdown" v-bind:data-var="'edit_z_t:'+propf+':t'" data-list="list-kv" data-list-values="datatypes-kv" data-list-label="DataTypes" v-bind:data-context-callback="refname+':z_t_type:'+propf" >{{ $root.edit_z_t[propf]['t']['v'] }}</div>
							<div v-else>{{ $root.edit_z_t[ propf ]['t']['v'] }}</div>
						</div>
					</td>
					<td v-if="edit_field==''">
				...
				<tr v-if="$root.edit_z_t[ propf ]['t']['k']=='O'" >
					<td colspan="9" >
						<div style="margin-left:20px; border-left:1px dashed #ccc; padding:0px 10px; " >
							...
							<table class="table table-bordered table-striped table-sm w-auto">
								<tbody>
									...
									<tr v-for="tvp,ti in $root.edit_z_t[ propf ]['z']['z_o']" >
										<td>{{ tvp }}</td>
										<td><input type="text" v-model="$root.edit_z_t[ propf ]['z']['z_t'][ tvp ]['l']" class="form-control form-control-sm" ></td>
										<td><select v-model="$root.edit_z_t[ propf ]['z']['z_t'][ tvp ]['t']" class="form-select form-select-sm" >
							...
							<table v-else  class="table table-bordered table-striped table-sm w-auto">
								<tbody>
									...
									<tr v-for="tvp,ti in $root.edit_z_t[ propf ]['z']['z_o']" >
										<td>{{ tvp }}</td>
										<td>{{ $root.edit_z_t[ propf ]['z']['z_t'][ tvp ]['l'] }}</td>
										<td>{{ $root.edit_z_t[ propf ]['z']['z_t'][ tvp ]['t'] }}</td>
									</tr>
								</tbody>
							</table>
						</div>
			...
			<table class="table table-bordered table-sm w-auto" >
				...
				<tr>
					...
					<template v-if="'z_o' in v" >
						<th v-for="fd in $root.thing['z_o']"><span v-if="fd in $root.thing['z_t']" >{{ v['z_t'][fd]['l']['v'] }}</span><span v-else>{{ fd }}</span></th>
					</template>
				</tr>
				</thead>
				<tbody>
