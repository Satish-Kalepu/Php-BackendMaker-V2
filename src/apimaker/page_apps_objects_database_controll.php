...
if( $_POST['action'] == "get_editor_graph_key" ){
	...
	$d = json_decode('{
	  ...
	  "policies": [
	    {
	      ...
	      "things": [
	        {
	          "thing": "*",
	          "_id": "storage_vault:'.$graph['settings']['library']['vault_id'].'"
	        }
	      ],
	      "records": [
	        "'.$graph['settings']['library']['dest_path'].'"
	      ]
	    },
	    {
	      "service": "objects",
	  ...
	  "expire_utc": "'.gmdate("Y-m-d H:i:s",time()+3600).'",
	  "expiret": {
	    "$date": {
	      "$numberLong": '.gmdate("U",time()+3600).'000
	    }
	  },
	  "t": "uk",
	  "hits": 0,
	  "maxhits": 20,
	  "hitsmin": 2,
	  "updated": "'.date("Y-m-d H:i:s",time()+3600).'",
	  "last_ip": "'.$_SERVER['REMOTE_ADDR'].'",
	  "last_used": '.time().'
	}',true);

	$ires = $mongodb_con->insert( $db_prefix . "_user_keys", $d );
...
if( $_GET['action'] == "load_storage_vaults" ){

	$res = $mongodb_con->find( $db_prefix . "_storage_vaults", [
		"app_id"=>$config_param1
	], [
		'sort'=>["des"=>1],
		'projection'=>['des'=>true,'vault_type'=>true],
	] );
	foreach( $res['data'] as $i=>$j ){
		if( $res['data'][$i]['vault_type'] == "AWS-S3" ){
			unset($res['data'][$i]['details']['key']);
			unset($res['data'][$i]['details']['secret']);
			unset($res['data'][$i]['details']['region']);
		}
	}
	json_response($res);
...
if( $_POST['action'] == "assume_upload_key" ){
	$resp = curl_post($config_settings['auth_api'], [
		"action"=> "get_session_key",
		"role_id"=> $config_settings['role_id'],
		"expire_type"=> "In", //In,At
		"expire_minutes"=> 2,
		"max_hits"=> 1,
		"client_ip"=> $_SERVER['REMOTE_ADDR'] . "/32",
		"hits_per_minute"=> 2
	], [
		"Access-Key: 671016b851544b5f140c818a", 
...
if( $_POST['action'] == "objects_load_object" ){
	if( !isset($_POST['object_id']) ){
		json_response(["status"=>"fail", "error"=>"Object Id Invalid" ]);
	}
	if( !preg_match("/^[a-z0-9]{2,24}$/i", $_POST['object_id']) && !preg_match("/^[0-9]+$/i", $_POST['object_id']) ){
		json_response(["status"=>"fail", "error"=>"Object Id Invalid" ]);
	}
...
if( $_POST['action'] == "objects_load_browse_list" ){
	...
	$res = $mongodb_con->find( $graph_things, $cond, [
		'projection'=>['l'=>1, 'i_t'=>1, 'ic'=>1, 'i_of'=>1, 'm_i'=>1, 'm_u'=>1,'cnt'=>1],
		'sort'=>$sort,
		'limit'=>100,
	]);
	$res['cnt'] = $cnt;
...
if( $_POST['action'] == "objects_create_node_on_fly" ){
	...
	if( !preg_match("/^[a-z0-9\-\_\.\,\ ]{2,200}$/i", $thing['l']['v']) ){
		json_response("fail", "Node Label invalid");
	}
	...
	if( !preg_match("/^[a-z0-9\-\_\.\,\ ]{2,200}$/i", $thing['i_of']['v']) ){
		json_response("fail", "Instance Label invalid");
	}
		...
		foreach( $res['data'] as $i=>$j ){
			...
			if( $ires['status'] == "success" ){
				if( $ires['inserted_id'] ){
					...
					send_to_keywords_queue( $ires['inserted_id'] );
...
if( $_POST['action'] == "objects_set_icon" ){
	if( !isset($_POST['object_id']) ){
		json_response("fail", "Need Thing id");
	}else if( !preg_match("/^[a-z0-9]{2,24}$/i", $_POST['object_id']) && !preg_match("/^[0-9]+$/i", $_POST['object_id']) ){
		json_response("fail", "Thing id incorrect");
	...
	}else if( !isset($_POST['ic']['t']) || !isset($_POST['ic']['v']) || !isset($_POST['ic']['it']) ){
		json_response("fail", "Incorrect data 1");
	}

	if( $_POST['ic']['t'] != "IC" ){
		json_response("fail", "Incorrect data 5");
	}
	if( !preg_match( "/^(emoji|svg|font|flag|img|img1|img2|img3|imgf)$/", $_POST['ic']['it'] ) ){
		json_response("fail", "Incorrect data 6");
	}
...
if( $_POST['action'] == "objects_save_object_html" ){
	if( !isset($_POST['object_id']) ){
		json_response("fail", "Need Thing id");
	}else if( !preg_match("/^[a-z0-9]{2,24}$/i", $_POST['object_id']) && !preg_match("/^[0-9]+$/i", $_POST['object_id']) ){
		json_response("fail", "Thing id incorrect");
...
if( $_POST['action'] == "save_settings_library" ){

	if( !isset($_POST['settings']) ){
		json_response("fail", "Need Settings");
	}else if( !isset($_POST['settings']['library']) || !isset($_POST['settings']['library_enable']) ){
		json_response("fail", "Incorrect settings format");
	}

	if( $_POST['settings']['library_enable'] === true ){
		$lib = $_POST['settings']['library'];
		if( !isset($lib['vault_id']) ){
			json_response("fail", "Incorrect settings: vault id missing");
		}
		if( !preg_match("/^[a-f0-9]{24}$/", $lib['vault_id']) ){
			json_response("fail", "Incorrect settings: incorrect vault id");
		}

		$storage_res = $mongodb_con->find_one($db_prefix . "_storage_vaults", [
			"app_id"=>$config_param1,
			"_id"=>$lib['vault_id']
		]);
		if( !$storage_res['data'] ){
			json_response("fail", "Incorrect settings: vault not found");
		}else{
			if( $storage_res['data']['vault_type'] == "AWS-S3" ){
				unset($storage_res['data']['details']['key']);
				unset($storage_res['data']['details']['secret']);
				unset($storage_res['data']['details']['region']);
			}else{
				json_response("fail", "Unknown storage type");
			}
		}
		$lib['vault'] = $storage_res['data'];

		if( !isset($lib['upload']) || !isset($lib['size']) || !isset($lib['dest_path']) || !isset($lib['thumb_path']) ){
			json_response("fail", "Incorrect settings: missing flags");
		}
		$lib['upload'] = $lib['upload']===true?true:false;
		if( !is_numeric($lib['size']) ){
			json_response("fail", "Incorrect settings: size");
		}else if( $lib['size'] < 1 || $lib['size'] > 50 ){
			json_response("fail", "Incorrect settings: size should be below 50MB");
		}
		if( !preg_match("/^\/[a-z0-9\/\+\-\_\.]{2,100}\/$/i", $lib['dest_path']) && !preg_match("/^\/$/i", $lib['dest_path']) ){
			json_response("fail", "Incorrect settings: destination path");
		}
		if( !preg_match("/^\/[a-z0-9\/\+\-\_\.]{2,100}\/$/i", $lib['thumb_path']) ){
			json_response("fail", "Incorrect settings: thumb path");
		}

		$fd = ltrim($lib['thumb_path'], "/");
		$fd = rtrim($fd, "/");
		$x = explode("/", $fd);
		$name = array_pop($x);
		$path = "/" . implode("/", $x);

		if( isset($graph['settings']['library']['file_id']) ){
			$mongodb_con->delete_one( $db_prefix . "_files", ["_id"=>$graph['settings']['library']['file_id']] );
		}

		$file_res = $mongodb_con->find_one( $db_prefix . "_files", [
			"app_id"=>$config_param1,
			"path"=>$path,
			"name"=>$name
		]);
		...
		if( $file_res['data'] ){
			if( $graph['settings']['library']['file_id'] == $file_res['data']['_id'] ){
				$file_settings_exists = true;
			}else{
				json_response("fail", "Thumb path conflict in files");
			}
		}

		if( !isset($lib['thing_id']) ){
			json_response("fail", "Incorrect settings: missing graph node id");
		}else if( !preg_match("/^[a-z0-9]{2,24}$/i", $lib['thing_id']) ){
			json_response("fail", "Incorrect settings: graph node id incorrect");
		}
		$tres = $mongodb_con->find_one( $graph_things, ["_id"=>$lib['thing_id']]);
		//json_response("fail", $tres);
		if( !$tres['data'] ){
			json_response("fail", "graph node not found");
		}
		if( !$tres['data']['z_t'] ){
			json_response("fail", "graph node properties incorrect format. Need P1:Description, P2:Keywords, P3:Filename");
		}
		if( !isset($tres['data']['z_t']['p1']) || !isset($tres['data']['z_t']['p2']) || !isset($tres['data']['z_t']['p3']) ){
			json_response("fail", "graph node properties incorrect format. Need P1:Description, P2:Keywords, P3:Filename");
		}
		if( $tres['data']['z_t']['p1']['l']['v'] != "Description" || $tres['data']['z_t']['p2']['l']['v'] != "Keywords" || $tres['data']['z_t']['p3']['l']['v'] != "Filename" ){
			json_response("fail", "graph node properties incorrect format. Need P1:Description, P2:Keywords, P3:Filename");
		}
		...
		$files_entry = [
			...
			"name"=>$name,
			"vault"=> [
				"vault_id"=> $lib['vault_id'],
				"vault_name"=> $storage_res['data']['des'],
				"vault_path"=> $lib['dest_path'],
				"local_path"=> $lib['thumb_path'],
				"vault_type"=> $storage_res['data']['vault_type']
			]
		];
		if( $file_settings_exists == false ){
			$ires = $mongodb_con->insert( $db_prefix . "_files", $files_entry );
				$ures = $mongodb_con->update_one( $db_prefix . "_graph_dbs", [
				"app_id"=>$config_param1,
				"_id"=>$graph_id
			], [
				'settings.library.file_id'=>$ires['inserted_id'],
			]);
		}else{
		...
		$res = $mongodb_con->update_one( $db_prefix . "_graph_dbs", [
			...
			'$unset'=>['settings.library'=>true],
			'$set'=>['settings.library_enable'=>false]
		]);
	}
